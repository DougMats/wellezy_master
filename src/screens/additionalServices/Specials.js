...
import { View, Text, StyleSheet, ActivityIndicator, Dimensions, TextInput, ScrollView, TouchableOpacity } from 'react-native';
...
function Specials(props) {
  ...
  async function get() {
    setLoad(true)
    const res = await specials.specialsList(i18n.language, search, page)
    setData(res)
    setLoad(false)
  }
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {Load === false && Data.data.length !== 0 && Data.data.map((i, key) => {
        return (
          <CardSpecials key={key} data={i} goToScreen={props.goToScreen} />
        )
      })
      }
